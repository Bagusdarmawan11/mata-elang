...
const LAYERS = [
  ...
  { key:'ships',      icon:'🚢', label:'Kapal Laut Live', countKey:'ships'   },
  { key:'quakes',     icon:'🌋', label:'Gempa (USGS)',    countKey:'quakes'  },
  { key:'bmkgQuakes', icon:'📡', label:'Gempa BMKG',      countKey:'bmkg'    },
  ...
  { key:'weather',    icon:'🌤', label:'Cuaca Indonesia', countKey:'weather' },
...
export default function Sidebar({open,layers,onToggleLayer,style,onStyleChange,onConfigOpen,flights,ships,quakes,bmkgQuakes,fires,weather,iss}){
  const getMeta=key=>{
    if(key==='flights')   return flights?.status==='error'?'rate limited':flights?.count?`${flights.count} aktif`:'memuat...'
    if(key==='ships')     return ships?.count?`${ships.count} kapal`:'butuh AISStream key'
...
const S={
  ...
  card:{display:'flex',alignItems:'center',gap:9,padding:'8px 10px',borderRadius:8,border:'1px solid transparent',cursor:'pointer',transition:'all .2s',marginBottom:3,background:'rgba(255,255,255,.02)'},
  cardOn:{borderColor:C.borderHi,background:C.cyanDim},
  icon:{width:30,height:30,borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,flexShrink:0,background:'rgba(255,255,255,.04)',border:`1px solid ${C.border}`,transition:'all .2s'},
